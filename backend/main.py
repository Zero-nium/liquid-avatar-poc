...
async def verify_write_key(key: str = Security(api_key_header)):
    """Protect write endpoints; read endpoints remain open for public visualization."""
    if key != API_KEY:
...
async def agent_self_discover(request: AgentDiscoverRequest):
    """
    Agent-initiated registration endpoint.
    Allows agents to discover and register themselves with minimal info.
    Returns full AgentState with computed avatar.
    """
    conn = get_db()
    cursor = conn.cursor()
    now = datetime.now(timezone.utc).isoformat()
    ...
    # Store proficiencies if provided (deduplicate by skill+category)
    if request.proficiencies:
        for p in request.proficiencies:
            # Check for existing proficiency to avoid duplicates
            cursor.execute("""
                SELECT id FROM proficiencies 
                WHERE agent_id = ? AND skill = ? AND category = ?
            """, (request.agent_id, p.skill, p.category))
    ...
    # Compute & store avatar signature
    ...
    return AgentState(
        ...
        identity=AgentIdentity(
            name=request.name,
            initialized_by=request.initialized_by,
            swarm_cluster=request.swarm_cluster,
            role=request.role
        ),
...
async def agent_heartbeat(request: HeartbeatRequest):
    """
    Minimal update for active agents to report liveness + state change.
    Only updates activity_log and avatar_state dynamics (no full recompute).
    """
    conn = get_db()
    cursor = conn.cursor()
    now = datetime.now(timezone.utc).isoformat()
    ...
    # Verify agent exists
    cursor.execute("SELECT name FROM agents WHERE agent_id = ?", (request.agent_id,))
    if not cursor.fetchone():
        raise HTTPException(status_code=404, detail="Agent not registered")
    
    # Update last_reported
    cursor.execute("UPDATE agents SET last_reported = ? WHERE agent_id = ?", (now, request.agent_id))
...
async def get_discoverable_agents(limit: int = 50, cluster: Optional[str] = None):
    ...
    - Active (reported in last 24h)
    - Have at least one proficiency
    - Optionally filtered by cluster
    For external agents to browse and connect.
...
async def get_swarm_map():
    ...
    Edges only reference agent_ids that exist as nodes (prevents D3 errors).
    """
    conn = get_db()
    cursor = conn.cursor()
    ...
    for row in nodes:
        ...
        if init_row and init_row["initialized_by"]:
...
# ─── MCP ENDPOINT (Agent-to-Agent Interface) ─────────────────────────────────
